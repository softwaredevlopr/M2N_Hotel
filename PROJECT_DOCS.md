...
- ✅ **Phase 10I — Persistent inventory dates** — migration `005`
  ...
  Smoke: `npm run verify:phase10i`. Admin write APIs + day-edit UI on
  `/admin/inventory` (`verify:inventory-dates`).
...
- ✅ **Phase 11 — Broader guest journey polish** — durable post-booking
...
- ✅ **Phase 13 — CRM Lite guest search + Guest 360 (first slice)** —
  hotel-scoped read model over `bookings` + `inquiries`. No guests table.
